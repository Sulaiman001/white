var tmpid=42,list="public",connected=!1,reConnecting=!1,room="",username="",conn=null,p=function(a){console.log(a)},removeTitle=function(){return"Remove item"},strikeTitle=function(){return"Strike out item"},saveText=function(a,c){$.getJSON("ajax.php?a=save&id="+a+"&text="+encodeURIComponent(c)+"&list="+encodeURIComponent(list),function(b){null===a?($("#wt-list-item-0").after('<div id="wt-list-item-'+b.id+'" class="wt-list-item" data-id="'+b.id+'"><p class="wt-list-item-text"><input type="checkbox" data-id="'+
b.id+'" class="wt-list-item-chk-done btn-tooltip" id="wt-list-item-chk-done-'+b.id+'" title="'+removeTitle()+'" /> <input type="checkbox" data-id="'+b.id+'" class="wt-list-item-chk-strike btn-tooltip" id="wt-list-item-chk-strike-'+b.id+'" title="'+strikeTitle()+'" /> <span id="wt-text-'+b.id+'" class="wt-text" data-id="'+b.id+'">'+c+"</span></p></div>"),conn.send(JSON.stringify({a:"message",actiontype:"add",list:list,text:c,id:b.id})),applyEditItem(b.id),applyRemoveItem(b.id),applyStrikeItem(b.id),
applySaveTextOfItem(b.id),applySaveOnEnter(b.id),applyTooltip()):conn.send(JSON.stringify({a:"message",actiontype:"save",list:list,text:c,id:b.id}))})},editing=!1,applyEditItem=function(a){$("#wt-text-"+a).on("click",function(){editing=!0;var a=$(this).data("id"),b=$("#wt-text-"+a).text();$("#wt-list-item-"+a).html('<input class="wt-list-item-input" type="text" id="wt-list-item-input-'+a+'" data-id="'+a+'" placeholder="Enter new item here" value="'+b+'"/>');$("#wt-list-item-input-"+a).focus();applySaveTextOfItem(a);
applySaveOnEnter(a);applyTooltip()})},applyRemoveItem=function(a){$("#wt-list-item-chk-done-"+a).on("click",function(){var a=$(this).data("id");$.getJSON("ajax.php?a=delete&id="+a+"&list="+encodeURIComponent(list),function(b){$("#wt-list-item-"+a).remove();conn.send(JSON.stringify({a:"message",actiontype:"remove",list:list,id:a}))})})},applyStrikeItem=function(a){$("#wt-list-item-chk-strike-"+a).on("click",function(){var a=$(this).data("id"),b=$("#wt-text-"+a),e=!1;b.hasClass("wt-strike")||(e=!0);
$.getJSON("ajax.php?a=strike&id="+a+"&strike="+e+"&list="+encodeURIComponent(list),function(k){e?b.addClass("wt-strike"):b.removeClass("wt-strike");conn.send(JSON.stringify({a:"message",actiontype:"strike",list:list,strike:e,id:a}))})})},applySaveTextOfItem=function(a){prevtime=parseInt((new Date).getTime());curval="";t=null;$(document).on("keyup","#wt-list-item-input-"+a,function(){var a=$(this).data("id");if(0!==a&&(curtime=parseInt((new Date).getTime()),next=prevtime+500,prevtime=curtime,curtime<
next)){clearTimeout(t);var b=$("#wt-list-item-input-"+a).val(),b=b.replace(/'/,"\\'");t=setTimeout("saveText('"+a+"', '"+b+"')",500)}})},doApplySaveOnEnter=function(a,c){if(editing){editing=!1;c.preventDefault();var b=a.data("id"),e=$("#wt-list-item-input-"+b).val();0===b?saveText(null,e):($("#wt-list-item-"+b).html('<p class="wt-list-item-text"><input type="checkbox" data-id="'+b+'" class="wt-list-item-chk-done btn-tooltip" id="wt-list-item-chk-done-'+b+'" title="'+removeTitle()+'" /> <input type="checkbox" data-id="'+
b+'" class="wt-list-item-chk-strike btn-tooltip" id="wt-list-item-chk-strike-'+b+'" title="'+strikeTitle()+'" /> <span id="wt-text-'+b+'" class="wt-text" data-id="'+b+'">'+e+"</span></p>"),applyEditItem(b),applyRemoveItem(b),applyStrikeItem(b),applySaveTextOfItem(b),applySaveOnEnter(b),applyTooltip(),saveText(b,e));$(this).val("")}},applySaveOnEnter=function(a){$("#wt-list-item-input-"+a).keypress(function(a){var b=$(this);b.on("blur",function(){doApplySaveOnEnter(b,a)});13==a.which&&doApplySaveOnEnter(b,
a)})},applyTooltip=function(){/iPhone|iPod|iPad|Android|BlackBerry|phone/i.test(navigator.userAgent)||$(".btn-tooltip").tooltip()},init=function(){var a=window.location.hash,a=a.replace(/^#/,""),a=a.split("/");switch(a[1]){case "list":list=a[2];startConnection(list);load(list);break;default:list="public",startConnection(list),load(list)}},load=function(a){$(".wt-list-item").remove();$("title").text("#"+a);$.getJSON("ajax.php?a=load&list="+encodeURIComponent(a),function(a){var b=0;$.each(a.items,function(a,
...
'" /> <span id="wt-text-'+d+'" class="wt-text '+g+'" data-id="'+d+'">'+f+"</span></p></div>");applyEditItem(d);applyRemoveItem(d);applyStrikeItem(d);applyTooltip();b=d})})};
