...
url:"services/save/"+encodeURIComponent(list)+"/"+a+"/"+b+"/"+encodeURIComponent(getHashVar(3)),data:{text:c},dataType:"json",success:function(b){moveListItemToTop(list);localStorage.removeItem(loadKey);if(null===a){var e=new Markon;$("#wt-list-item-0").after('<div id="wt-list-item-'+b.id+'" class="wt-list-item" data-id="'+b.id+'"><p class="wt-list-item-text">'+buildDoneCheckbox(b.id)+" "+buildStrikeCheckbox(b.id)+' <span id="wt-text-'+b.id+'" class="wt-text" data-id="'+b.id+'">'+e.render(escapeHtml(c))+
...
a).html('<input class="wt-list-item-input" type="text" id="wt-list-item-input-'+a+'" data-id="'+a+'" placeholder="Enter new item here" />');$("#wt-list-item-input-"+a).val(b.unRender(d));$("#wt-list-item-input-"+a).focus();applySaveTextOfItem(a);applySaveOnEnter(a);applyTooltip()})},applyRemoveItem=function(a){$("#wt-list-item-chk-done-"+a).on("click",function(){if(confirm("Confirm delete please")){var a=$(this).data("id");$.getJSON("services/delete/"+a+"/"+encodeURIComponent(getHashVar(3)),function(b){moveListItemToTop(list);
localStorage.removeItem(loadKey);$("#wt-list-item-"+a).remove();addStrikeHeader();useWebSockets&&conn.send(JSON.stringify({a:"message",actiontype:"remove",list:list,id:a}))})}})},applyStrikeItem=function(a){$("#wt-list-item-chk-strike-"+a).on("click",function(){var a=$(this).data("id"),b=$("#wt-text-"+a),d=!1;b.hasClass("wt-strike")||(d=!0);$("#wt-list-item-chk-strike-"+a);d?b.addClass("wt-strike"):b.removeClass("wt-strike");$.getJSON("services/strike/"+a+"/"+d+"/"+encodeURIComponent(getHashVar(3)),
function(b){moveListItemToTop(list);localStorage.removeItem(loadKey);b=$("#wt-list-item-"+a);d?$(".wt-list-item:last").after(b):0<$(".wt-strike").size()&&$(".wt-strike:first").parent().parent().before(b);addStrikeHeader(!0);useWebSockets&&conn.send(JSON.stringify({a:"message",actiontype:"strike",list:list,strike:d,id:a}))}).fail(function(){d?b.removeClass("wt-strike"):b.addClass("wt-strike")})})},applySaveTextOfItem=function(a){prevtime=parseInt((new Date).getTime());curval="";t=null;$(document).on("keyup",
"#wt-list-item-input-"+a,function(){var a=$(this).data("id");if(0!==a&&(curtime=parseInt((new Date).getTime()),next=prevtime+500,prevtime=curtime,curtime<next)){clearTimeout(t);var b=$("#wt-list-item-input-"+a).val(),b=b.replace(/'/,"\\'");t=setTimeout("saveText('"+a+"', '"+b+"', 'false')",500)}})},doApplySaveOnEnter=function(a,c){if(editing||$("#wt-list-item-input-0").is(":focus")){editing=!1;c.preventDefault();var b=a.data("id"),d=$("#wt-list-item-input-"+b).val();if(0===b)saveText(null,d,!0);else{var e=
new Markon;$("#wt-list-item-"+b).html('<p class="wt-list-item-text">'+buildDoneCheckbox(b)+" "+buildStrikeCheckbox(b)+' <span id="wt-text-'+b+'" class="wt-text" data-id="'+b+'">'+e.render(escapeHtml(d))+"</span></p>");applyEditItem(b);applyRemoveItem(b);applyStrikeItem(b);applySaveTextOfItem(b);applySaveOnEnter(b);applyTooltip();saveText(b,d,!0)}a.val("")}},applySaveOnEnter=function(a){$("#wt-list-item-input-"+a).keypress(function(a){var b=$(this);b.on("blur",function(){doApplySaveOnEnter(b,a)});
13!==a.which&&27!==a.which||doApplySaveOnEnter(b,a)})},applyAllListClick=function(){$(".wt-all-list-item").on("click",function(){window.location.hash="#/list/"+$(this).data("list")+"/"+getHashVar(2)})},applyTooltip=function(){/iPhone|iPod|iPad|Android|BlackBerry|phone/i.test(navigator.userAgent)||$(".btn-tooltip").tooltip()},menuToggle=function(){$(".menu-toggle").unbind("click");$(".menu-toggle").on("click",function(a){a.preventDefault();a=$("#wrapper");a.hasClass("toggled")?a.removeClass("toggled"):
a.addClass("toggled")})},hideSideBar=function(){$("#wrapper").removeClass("toggled")},setListsLink=function(a){$(".lists-link").attr("href","#/lists/"+encodeURIComponent(a))},init=function(){var a=window.location.hash,a=a.replace(/^#/,""),a=a.split("/");startConnection();switch(a[1]){case "list":list=a[2];loadKey="list-"+list;load(list);seedSideBar(a[3]);setListsLink(a[3]);setPlaceholder(list);break;case "lists":loadAll();seedSideBar(a[2]);setListsLink(a[2]);break;default:window.location.hash="#/list/inbox/"+
a[1]}menuToggle()},isUndefined=function(a){return void 0===a},getHashVar=function(a){return window.location.hash.replace(/^#/,"").split("/")[a]},loadAll=function(){$("#wt-list-item-0").hide();$(".wt-list-item").remove();$("title").text("all lists");$.getJSON("services/load-all/"+encodeURIComponent(getHashVar(2)),function(a){var c=$(a.items).size(),b=Math.floor(c/4),d=c-3*b,e=$(".wt-list");4<=c?(e.append("<div class='row'><div class='col-sm-12 col-md-6 col-lg-3' id='list-col-1'></div><div class='col-sm-12 col-md-6 col-lg-3' id='list-col-2'></div><div class='col-sm-12 col-md-6 col-lg-3' id='list-col-3'></div><div class='col-sm-12 col-md-6 col-lg-3' id='list-col-4'></div></div>"),
...
function(a){addListItem(a.items)})},addListItem=function(a){var c=new Markon,b=0;$.each(a,function(a,e){var f=e.id,g=e.text;0===a&&(b=0);var h=e.strike?"wt-strike":"";$("#wt-list-item-"+b).after('<div id="wt-list-item-'+f+'" class="wt-list-item" data-id="'+f+'"><p class="wt-list-item-text">'+buildDoneCheckbox(f)+" "+buildStrikeCheckbox(f)+' <span id="wt-text-'+f+'" class="wt-text '+h+'" data-id="'+f+'">'+c.render(escapeHtml(g))+'</span> <span class="timestamp">'+e.timestamp+"</span></p></div>");applyEditItem(f);
