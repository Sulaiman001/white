...
escapeHtml(b)+'"><a title="#'+escapeHtml(b)+'" href="#/list/'+escapeHtml(b)+"/"+getHashVar(2)+'">#'+escapeHtml(b)+"</a></div>")}));applyAllListClick();addStrikeHeader()})},seedSideBar=function(a){$.getJSON("services/load-all/"+encodeURIComponent(a),function(c){$.each(c.items,function(b,c){$(".sidebar-nav").append('<li class="wt-all-list-item-li"><div class="wt-all-list-item" data-list="'+escapeHtml(c)+'"><a class="wt-all-list-item-a" title="#'+escapeHtml(c)+'" href="#/list/'+escapeHtml(c)+"/"+encodeURIComponent(a)+
'">#'+escapeHtml(c)+"</a></div></li>")});sortSideBar("list-name","asc")});hideSideBar()},sortSideBar=function(a,c){var b=$(".wt-all-list-item-li");"list-name"===a&&"asc"===c&&b.sort(function(a,b){var c=$(a).find(".wt-all-list-item-a").text().replace(/^#/,"")||"",g=$(g).find(".wt-all-list-item-a").text().replace(/^#/,"")||"";return c.localeCompare(g)})},escapeDoubleQuotes=function(a){return void 0===a||null===a?a:a.replace(/"/,'\\"')},buildPlaceholder=function(a){return"Add to #"+a},setPlaceholder=
function(a){$("#wt-list-item-input-0").attr("placeholder",escapeDoubleQuotes(buildPlaceholder(a)))},load=function(a){$("#wt-list-item-0").show();$(".wt-list-item").remove();$(".wt-all-list-item").remove();$("title").text("#"+a);var c="list-"+a;if(null!=localStorage.getItem(c)){var b=JSON.parse(localStorage.getItem(c));addListItem(b.items);addStrikeHeader(!0)}else $.getJSON("services/load/"+encodeURIComponent(a)+"/"+encodeURIComponent(getHashVar(3)),function(b){null==localStorage.getItem(c)&&localStorage.setItem(c,
JSON.stringify(b));addListItem(b.items);addStrikeHeader(!0);setPlaceholder(a)}).fail(function(){if(null!=localStorage.getItem(c)){var a=JSON.parse(localStorage.getItem(c));addListItem(a.items);addStrikeHeader(!0)}})},loadItem=function(a,c,b){$.getJSON("services/load/"+encodeURIComponent(c)+"/"+a+"/"+encodeURIComponent(getHashVar(3)),function(d){0<d.items.length?b(d.items[0]):alert("Could not find list item "+a+" for list "+c)}).fail(function(){alert("Could not load list item "+a+" for list "+c)})},
search=function(a){$(".wt-list-item").remove();$.getJSON("services/search/"+encodeURIComponent(a)+"/"+encodeURIComponent(getHashVar(3)),function(a){addListItem(a.items)})},addListItem=function(a){var c=new Markon,b=0;$.each(a,function(a,e){var f=e.id,g=e.text;0===a&&(b=0);var h=e.strike?"wt-strike":"";$("#wt-list-item-"+b).after('<div id="wt-list-item-'+f+'" class="wt-list-item" data-id="'+f+'"><p class="wt-list-item-text">'+buildDoneCheckbox(f)+" "+buildStrikeCheckbox(f)+' <span id="wt-text-'+f+
'" class="wt-text '+h+'" data-id="'+f+'">'+c.render(escapeHtml(g))+'</span> <span class="timestamp">'+e.timestamp+"</span></p></div>");applyEditItem(f);applyRemoveItem(f);applyStrikeItem(f);applyTooltip();b=f})},sortByPriority=function(){},handleMessage=function(a){a=JSON.parse(a);if("message"===a.a&&a.list===list)if("strike"===a.actiontype){var c=$("#wt-text-"+a.id);a.strike?c.addClass("wt-strike"):c.removeClass("wt-strike");$("#wt-list-item-chk-strike-"+a.id).prop("checked",a.strike)}else"remove"===
a.actiontype?$("#wt-list-item-"+a.id).remove():"add"===a.actiontype?($("#wt-list-item-0").after('<div id="wt-list-item-'+a.id+'" class="wt-list-item" data-id="'+a.id+'"><p class="wt-list-item-text">'+buildDoneCheckbox(a.id)+" "+buildStrikeCheckbox(a.id)+' <span id="wt-text-'+a.id+'" class="wt-text" data-id="'+a.id+'">'+a.text+"</span></p></div>"),applyEditItem(a.id),applyRemoveItem(a.id),applyStrikeItem(a.id),applySaveTextOfItem(a.id),applySaveOnEnter(a.id),applyTooltip()):"save"===a.actiontype&&
...
" "))},startConnection=function(){useWebSockets&&!connected&&(conn=new WebSocket(webSocketUrl),conn.onopen=function(a){connected=!0;reConnecting=!1},conn.onclose=function(a){connected=!1;reConnecting||reConnect()},conn.onerror=function(a){connected=!1;reConnecting||reConnect()},conn.onmessage=function(a){handleMessage(a.data)})},reConnect=function(){reConnecting=!0;connected||(init(),setTimeout(reConnect,2E3))};function Markon(){}Markon.prototype.render=function(a){return this.mutate(a,[this.code])};
Markon.prototype.unRender=function(a){return this.mutate(a,[this.unCode])};Markon.prototype.mutate=function(a,c){var b=a;for(m=0;m<c.length;m++)b=c[m](b);return b};Markon.prototype.code=function(a){return void 0===a||null===a||0===a.length?a:a.replace(/`(.*?)`/g,"<code>$1</code>")};Markon.prototype.unCode=function(a){return void 0===a||null===a||0===a.length?a:a.replace(/<code>(.*?)<\/code>/g,"`$1`")};
