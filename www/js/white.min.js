...
applyStrikeItem(b),applySaveTextOfItem(b),applySaveOnEnter(b),applyTooltip(),saveText(b,e));a.val("")}},applySaveOnEnter=function(a){$("#wt-list-item-input-"+a).keypress(function(a){var b=$(this);b.on("blur",function(){doApplySaveOnEnter(b,a)});13==a.which&&doApplySaveOnEnter(b,a)})},applyTooltip=function(){/iPhone|iPod|iPad|Android|BlackBerry|phone/i.test(navigator.userAgent)||$(".btn-tooltip").tooltip()},init=function(){var a=window.location.hash,a=a.replace(/^#/,""),a=a.split("/");switch(a[1]){case "list":list=
a[2];startConnection(list);load(list);break;default:list="public",startConnection(list),load(list)}},load=function(a){$(".wt-list-item").remove();$("title").text("#"+a);$.getJSON("ajax.php?a=load&list="+encodeURIComponent(a),function(a){var b=0;$.each(a.items,function(a,c){var d=c.id,f=c.text;0===a&&(b=0);var g=c.strike?"wt-strike":"",h=c.strike?'checked="checked"':"";$("#wt-list-item-"+b).after('<div id="wt-list-item-'+d+'" class="wt-list-item" data-id="'+d+'"><p class="wt-list-item-text"><input type="checkbox" data-id="'+
