var list="inbox",loadKey=list,connected=!1,reConnecting=!1,room="",username="",conn=null,autolinker=new Autolinker({stripPrefix:!1,newWindow:!0}),prevtime=new Date,curval,p=function(a){},removeTitle=function(){return"Remove item"},strikeTitle=function(){return"Strike out item"},escapeHtml=function(a){a=autolinker.link(jQuery("<div/>").text(a).html());a=a.replace(/@&lt;(.*?)&gt;/,'<span class="label label-remind">$1</span>');a=a.replace(/@\((.*?)\)/,'<span class="label label-remind">$1</span>');a=
a.replace(/([0-9]{1,2}:[0-9]{1,2} [0-9]{1,2}\/[0-9]{1,2}\/[0-9]{4})/,'<span class="label label-remind">$1</span>');a=a.replace(/([^\\])?(#bug)/g,'$1<span class="label label-bug">$2</span>');a=a.replace(/([^\\])?(#feature)/g,'$1<span class="label label-feature">$2</span>');a=a.replace(/([^\\])?(#improvement)/g,'$1<span class="label label-improvement">$2</span>');a=a.replace(/([^\\])?(#enhancement)/g,'$1<span class="label label-enhancement">$2</span>');a=a.replace(/([^\\])?(#[a-zA-Z0-9-_]+)/g,'$1<span class="label label-label">$2</span>');
...
addStrikeHeader(!0)}else $.getJSON("services/load/"+encodeURIComponent(a)+"/"+encodeURIComponent(getHashVar(3)),function(b){null==localStorage.getItem(c)&&localStorage.setItem(c,JSON.stringify(b));addListItem(b.items);addStrikeHeader(!0);setPlaceholder(a)}).fail(function(){if(null!=localStorage.getItem(c)){var a=JSON.parse(localStorage.getItem(c));addListItem(a.items);addStrikeHeader(!0)}})},loadItem=function(a,c,b){$.getJSON("services/load/"+encodeURIComponent(c)+"/"+encodeURIComponent(getHashVar(3)),
function(d){0<d.items.length?b(d.items[0]):alert("Could not find list item "+a+" for list "+c)}).fail(function(){alert("Could not load list item "+a+" for list "+c)})},search=function(a){$(".wt-list-item").remove();$.getJSON("services/search/"+encodeURIComponent(a)+"/"+encodeURIComponent(getHashVar(3)),function(a){addListItem(a.items)})},addListItem=function(a){var c=new Markon,b=0;$.each(a,function(a,e){var f=e.id,g=e.text;0===a&&(b=0);var h=e.strike?"wt-strike":"";$("#wt-list-item-"+b).after('<div id="wt-list-item-'+
