...
escapeHtml(b)+'"><a title="#'+escapeHtml(b)+'" href="#/list/'+escapeHtml(b)+"/"+getHashVar(2)+'">#'+escapeHtml(b)+"</a></div>")}));applyAllListClick();addStrikeHeader()})},seedSideBar=function(a){$.getJSON("services/load-all/"+encodeURIComponent(a),function(c){c=$.map(c.items,function(a,b){return[a]});for(var b="#e22d2d #e5ba1d #dde23b #21c621 #3a82e0 #a43ddb".split(" "),d=Math.ceil(c.length/b.length),e=0,f=0;f<c.length;f+=d){0===f/10%d&&(e=0);var g=c.slice(f,f+d);$.each(g,function(c,d){var f=" style = 'color:"+
b[e]+";' ";$(".sidebar-nav").append('<li class="wt-all-list-item-li"><div class="wt-all-list-item" data-list="'+escapeHtml(d)+'"><a class="wt-all-list-item-a" '+f+' title="#'+escapeHtml(d)+'" href="#/list/'+escapeHtml(d)+"/"+encodeURIComponent(a)+'">#'+escapeHtml(d)+"</a></div></li>")});e++}sortSideBar("list-name","asc")});hideSideBar()},sortSideBar=function(a,c){var b=$(".wt-all-list-item-li");"list-name"===a&&"asc"===c&&b.sort(function(a,b){var c=$(a).find(".wt-all-list-item-a").text().replace(/^#/,
"")||"",g=$(g).find(".wt-all-list-item-a").text().replace(/^#/,"")||"";return c.localeCompare(g)})},escapeDoubleQuotes=function(a){return void 0===a||null===a?a:a.replace(/"/,'\\"')},buildPlaceholder=function(a){return"Add to #"+a},setPlaceholder=function(a){$("#wt-list-item-input-0").attr("placeholder",escapeDoubleQuotes(buildPlaceholder(a)))},load=function(a){$("#wt-list-item-0").show();$(".wt-list-item").remove();$(".wt-all-list-item").remove();$("title").text("#"+a);var c="list-"+a;if(null!=localStorage.getItem(c)){var b=
JSON.parse(localStorage.getItem(c));addListItem(b.items);addStrikeHeader(!0)}else $.getJSON("services/load/"+encodeURIComponent(a)+"/"+encodeURIComponent(getHashVar(3)),function(b){null==localStorage.getItem(c)&&localStorage.setItem(c,JSON.stringify(b));addListItem(b.items);addStrikeHeader(!0);setPlaceholder(a)}).fail(function(){if(null!=localStorage.getItem(c)){var a=JSON.parse(localStorage.getItem(c));addListItem(a.items);addStrikeHeader(!0)}})},loadItem=function(a,c,b){$.getJSON("services/load/"+
encodeURIComponent(c)+"/"+a+"/"+encodeURIComponent(getHashVar(3)),function(d){0<d.items.length?b(d.items[0]):alert("Could not find list item "+a+" for list "+c)}).fail(function(){alert("Could not load list item "+a+" for list "+c)})},search=function(a){$(".wt-list-item").remove();$.getJSON("services/search/"+encodeURIComponent(a)+"/"+encodeURIComponent(getHashVar(3)),function(a){addListItem(a.items)})},addListItem=function(a){var c=new Markon,b=0;$.each(a,function(a,e){var f=e.id,g=e.text;0===a&&
(b=0);var h=e.strike?"wt-strike":"";$("#wt-list-item-"+b).after('<div id="wt-list-item-'+f+'" class="wt-list-item" data-id="'+f+'"><p class="wt-list-item-text">'+buildDoneCheckbox(f)+" "+buildStrikeCheckbox(f)+' <span id="wt-text-'+f+'" class="wt-text '+h+'" data-id="'+f+'">'+c.render(escapeHtml(g))+'</span> <span class="timestamp">'+e.timestamp+"</span></p></div>");applyEditItem(f);applyRemoveItem(f);applyStrikeItem(f);applyTooltip();b=f})},sortByPriority=function(){},handleMessage=function(a){a=
